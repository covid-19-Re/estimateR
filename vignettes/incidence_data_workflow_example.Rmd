...
EST_incidence <- EST_data %>%
  transmute(report_date = as_date(StatisticsDate),
            incidence = DailyCases)
}
```

In case the data is innacessible, an earlier version is stored in the `EST_incidence_data` variable with **estimateR**.
```{r}
...
EST_incidence <- EST_incidence %>% 
  ...
  mutate(incidence = if_else(incidence < 0, 0, incidence)) %>% 
  #replace potential NA values by 0
   replace_na(list(incidence = 0)) %>%
  # add zeroes for dates with no reported case
    complete(report_date = seq.Date(min(report_date),  max(report_date),
                                      by = "days"),
...
ref_date = min(EST_incidence$report_date)
time_step = "day"
```
...
EST_estimates <- get_block_bootstrapped_estimate(
  incidence_data = EST_incidence$incidence,
...
```{r, fig.width=8, fig.height=4}
ggplot(filter(EST_incidence, report_date < as.Date("2020-09-01"), report_date >= as.Date("2020-05-01")), aes(x = report_date, y = incidence)) +
