...
This is normal sincea case counted on day X in the *case_incidence* column could appear on, say, day X - 3 in the *onset_incidence* column.
...
The *estimation_window* window parameter specifies the number of time steps (days here) over which the reproductive number is assumed to be constant.
...
they first need to be confirmed as a positive case, which usually happens later than day T (say T + 3 days).
...
but this time we directly feed the *HK_delay_data* dataframe to the *delay_onset_to_report* of  *get_block_bootstrapped_estimate*.
...
We can do the same modification to the analysis above that included symptom onset data:
